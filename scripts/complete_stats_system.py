...
# -*- coding: utf-8 -*-
"""
MLB Complete Stats System - 完全版統計システム
データ不足投手の前年度成績表示を含む

実行: python -m scripts.complete_stats_system
"""
...
class CompleteStatsSystem:
    ...
    def _get_vs_lr_stats(self, pitcher_id, season):
        ...
        games = game_logs['stats'][0].get('splits', [])[:10]  # 最新10試合
    ...
    def get_today_games(self):
        ...
        today_est = (now_jst - timedelta(hours=13)).date()
        
        schedule = self.client._make_request(
            f"schedule?sportId=1&date={today_est}&hydrate=probablePitcher"
        )
    ...
    def run_daily_report(self):
        """日次レポートを実行"""
        print("MLB Complete Stats System - 日次レポート開始")
        ...
        print(f"{len(games)}試合を処理中...")
        
        for game in games[:3]:  # テスト用に3試合まで
