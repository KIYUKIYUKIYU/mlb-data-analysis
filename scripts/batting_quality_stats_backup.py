...
class BattingQualityStats:
    ...
    def calculate_woba(self, team_stats):
        ...
        try:
            # 必要な統計を取得
            bb = team_stats.get('baseOnBalls', 0)
            hbp = team_stats.get('hitByPitch', 0)
            hits = team_stats.get('hits', 0)
            doubles = team_stats.get('doubles', 0)
            triples = team_stats.get('triples', 0)
            hr = team_stats.get('homeRuns', 0)
            ab = team_stats.get('atBats', 0)
            sf = team_stats.get('sacFlies', 0)
            ...
            denominator = ab + bb - team_stats.get('intentionalWalks', 0) + sf + hbp
            ...
            # OPSとの相関から推定
            ops = team_stats.get('ops', 0.700)
            xwoba = 0.295 + (0.175 * ops)  # 簡易推定式
