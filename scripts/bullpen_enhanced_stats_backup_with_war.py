        ...
        try:
            ...
            for player in roster:
                ...
                if position_abbr == 'P':
                    ...
                    if player_id:
                        # 2025年の統計を取得
                        stats_data = self.api_client.get_player_stats_by_season(player_id, 2025)
                        ...
                        if stats_data and 'stats' in stats_data:
                            ...
                            if stats_list and len(stats_list) > 0:
                                ...
                                if 'splits' in first_stat and first_stat['splits']:
                                    stats = first_stat['splits'][0].get('stat', {})
            ...
            for reliever in relievers:
                ...
                if innings > 0:
                    ...
                    total_strikeouts += int(stats.get('strikeOuts', 0))
                    total_walks += int(stats.get('baseOnBalls', 0))
                    total_hits += int(stats.get('hits', 0))
                    total_hr += int(stats.get('homeRuns', 0))
                    total_games += int(stats.get('gamesPlayed', 0))
                    total_batters_faced += int(stats.get('battersFaced', 0))

                    # FIPを計算
                    hbp = int(stats.get('hitByPitch', 0))
                    fip = ((13 * total_hr) + (3 * (total_walks + hbp)) - (2 * total_strikeouts)) / innings + 3.20
            ...
            if total_innings > 0:
                ...
                hbp = 0  # 簡易版
                bullpen_fip = ((13 * hr9) + (3 * bb9) - (2 * k9)) / 9 + 3.20
            ...
            bullpen_xfip = ((13 * hr9_expected + 3 * bb9 - 2 * k9) / 9) + 3.20 if total_innings > 0 else 0.00
            ...
            result = {
                ...
                'war': f"{total_innings / 50:.1f}",  # 簡易計算
