...
class DiscordReportWithTable:
    ...
    def format_game_message(self, game_data):
        ...
        # Away Team
        message += f"**【{game_data.get('away_team', 'Unknown')}】**\n\n"
        
        # 1. 先発投手
        if game_data.get('away_pitcher'):
            p = game_data['away_pitcher']
            message += f"**先発**: {p['name']} ({p['wins']}勝{p['losses']}敗)\n"
            message += f"ERA: {p['era']} | FIP: {p['fip']} | WHIP: {p['whip']} | K-BB%: {p['k_bb']}\n\n"
        else:
            message += "**先発**: 未定\n\n"
        ...
        # Home Team（同じ順番で）
        message += f"**【{game_data.get('home_team', 'Unknown')}】**\n\n"
...
#                    'home_team': self.to_japanese_team(game['teams']['home']['team']['name']),
        ...
        for i, game in enumerate(games_data[:3]):
            print(f"試合 {i+1}/{min(3, len(games_data))}: {game.get('away_team', 'Unknown')} @ {game.get('home_team', 'Unknown')}")
    ...
    def to_japanese_team(self, team_name):
        """チーム名を日本語に変換"""
        return self.name_db.get_team_name(team_name)
    
    def to_japanese_player(self, player_name):
        """選手名を日本語に変換"""
        return self.name_db.get_player_name(player_name)
