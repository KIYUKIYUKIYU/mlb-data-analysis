...
# -*- coding: utf-8 -*-
"""
MLBレポートをHTMLに変換するスクリプト（目標例完全再現版）
- TailwindCSS使用
- ESPNチームロゴ
- 詳細な総括
- 1試合1ページレイアウト
...
def create_game_page(game_data):
    """1試合分のHTMLページを生成（目標例のレイアウトを完全再現）"""
    ...
    <div class="game-page">
        <header class="flex justify-between items-center pb-4 border-b-2 border-gray-300">
            <div class="team-info text-center w-1/3">
                <img src="{get_team_logo(away_team)}" alt="{away_team} Logo" class="w-12 h-12 mx-auto mb-2">
                <h1 class="text-3xl font-black">{away_team}</h1>
                {away_pitcher_html}
            </div>
            ...
            <div class="team-info text-center w-1/3">
                <img src="{get_team_logo(home_team)}" alt="{home_team} Logo" class="w-12 h-12 mx-auto mb-2">
                <h1 class="text-3xl font-black">{home_team}</h1>
                {home_pitcher_html}
            </div>
        </header>
