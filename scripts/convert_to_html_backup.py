...
def create_team_section(data, title):
    ...
    <div class="stats-category">
        ...
        <div class="pitcher-info">
            ...
            <span class="pitcher-hand" style="background-color: {hand_color}">{hand_text}</span>
        </div>
        <div class="stat-row">
...
def generate_summary(game_data):
    ...
    try:
        if away.get('ERA') and home.get('ERA'):
            ...
            if abs(away_era - home_era) > 1.0:
                better_team = game_data['away_team'] if away_era < home_era else game_data['home_team']
    ...
    # 打撃の調子
    try:
        if away.get('past5_OPS') and home.get('past5_OPS'):
            away_ops = float(away['past5_OPS'])
            home_ops = float(home['past5_OPS'])
            if away_ops > 0.800:
                summary.append(f"{game_data['away_team']}の打線が好調")
            if home_ops > 0.800:
                summary.append(f"{game_data['home_team']}の打線が好調")
    except:
        pass
    ...
    if not summary:
        ...
        summary.append(f"ホームの{game_data['home_team']}にわずかながらアドバンテージがある")
