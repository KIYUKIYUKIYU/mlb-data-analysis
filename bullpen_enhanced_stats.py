...
class BullpenEnhancedStats:
    ...
    def get_enhanced_bullpen_stats(self, team_id: int, date: str = None) -> Dict[str, Any]:
        ...
        try:
                ...
                if position_abbr == 'P':
                    ...
                    if player_id:
                        ...
                        if stats and isinstance(stats, dict) and len(stats) > 0:
                            ...
                            games_started = self._safe_int(stats.get('gamesStarted', 0))
                            games_played = self._safe_int(stats.get('gamesPlayed', 0))
            ...
            for reliever in relievers:
                ...
                # イニング数の変換
                innings = self._convert_innings(stats.get('inningsPitched', '0.0'))

                if innings > 0:
                    # 基本統計を集計
                    era = self._safe_float(stats.get('era', '0.00'))
                    total_era += era * innings
                    total_innings += innings
                    total_strikeouts += self._safe_int(stats.get('strikeOuts', 0))
                    total_walks += self._safe_int(stats.get('baseOnBalls', 0))
                    total_hits += self._safe_int(stats.get('hits', 0))
                    total_hr += self._safe_int(stats.get('homeRuns', 0))
                    total_games += self._safe_int(stats.get('gamesPlayed', 0))
                    total_batters_faced += self._safe_int(stats.get('battersFaced', 0))

                    # FIPを計算
                    hbp = self._safe_int(stats.get('hitByPitch', 0))
                    k = self._safe_int(stats.get('strikeOuts', 0))
                    bb = self._safe_int(stats.get('baseOnBalls', 0))
                    hr = self._safe_int(stats.get('homeRuns', 0))
                    ...
                    # 名前をASCII安全な形式に変換
                    safe_name = self._make_safe_name(reliever['name'])
                    ...
                    active_relievers.append({
                        ...
                        'fip': f"{fip:.2f}",
                        'innings': innings,
                        'saves': self._safe_int(stats.get('saves', 0)),
                        'holds': self._safe_int(stats.get('holds', 0))
                    })
                    ...
                    # 疲労度チェック（簡易版）
                    games = self._safe_int(stats.get('gamesPlayed', 0))
            ...
            # キャッシュに保存（ensure_ascii=Falseでユニコード文字を保持）
    ...
    def _convert_innings(self, innings_str):
        """イニング数を数値に変換"""
        try:
